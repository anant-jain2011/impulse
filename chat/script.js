...
function decodeMessage(message, dateStr) {
    ...
    console.log("decoding message", message, date);
...
const displayMessage = (content, position, id, sender) => {
...
window.onload = async () => {
        ...
        messages.map((message) => {
            if (message.roomNumber == roomNumber) {
                if (message.sentFrom != username) {
                    displayMessage(decodeMessage(message.content, message.createdAt), "left", message._id, message.sentFrom);
                }
    ...
    eventSource.addEventListener("message", (event) => {
        ...
        if (item.sentFrom != username && item.roomNumber == roomNumber) {
            displayMessage(decodeMessage(item.content, item.createdAt), "left", item._id, item.sentFrom);
